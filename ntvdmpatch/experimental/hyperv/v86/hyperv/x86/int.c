...
--*/
{
    /* CRITICAL: If we're called from a worker thread (timer / event /
     * mouse / com / etc.) while the main thread is inside
     * WHvRunVirtualProcessor, the main thread won't notice the
     * pNtVDMState change we're about to make until WHvRunVirtualProcessor
     * returns on its own.  If the VCPU is in HLT (IF=0) or in a tight
     * CPU-bound loop with no I/O / BOPs / exceptions, that return never
     * happens.  HAXM solved this in the kernel driver by injecting
     * synchronously from any thread; WHP user-mode does not give us
     * that, so we force the main thread to exit WHvRunVirtualProcessor
     * with WHvRunVpExitReasonCanceled.  The next cpu_simulate iteration
     * then sees VDM_INT_TIMER / VDM_INT_HARDWARE and drains it.
     *
     * Cheap to invoke (one WHvCancelRunVirtualProcessor hypercall per
     * IRQ queue, i.e. ~18 times/sec for the timer plus per-keypress).
     * Skip when called from the main thread itself -- it isn't inside
     * WHvRunVirtualProcessor when running C code like host_timer_event
     * or its callees, so no cancel needed. */
    {
        extern void hyperv_cancel_run_if_running(void); /* monitor.c */
...
#endif
		{
			/* WHP has exactly one WHvRegisterPendingInterruption slot.
			 * Only consume an IRQ from the PIC (via ica_intack, which
			 * sets ISR / clears IRR) when we know WHP can accept the
			 * injection.  If a previous injection is still un-delivered
			 * (VCPU in CLI / interrupt shadow), we leave both the PIC
			 * state and VDM_INT_HARDWARE untouched and retry on the
			 * next cpu_simulate loop iteration.
			 *
			 * HAXM didn't need this because its kernel driver maintained
			 * an internal queue of pending interrupts; WHP does not. */
			if (hyperv_pending_interruption_slot_free()
			    && hyperv_yoda_inject_allowed()) {
				ULONG IretHookAddress = 0L;
				int InterruptNumber = ica_intack(&IretHookAddress);
				if (InterruptNumber != -1)
					hyperv_inject_interrupt((uint32_t)InterruptNumber);
				*pNtVDMState &= ~VDM_INT_HARDWARE;
			}
			/* else: slot busy OR YODA tracing mid-IRQ -- leave
			 * VDM_INT_HARDWARE set, retry next loop.  Either way we
			 * MUST NOT call ica_intack here: that would advance the
			 * PIC's in-service state while no injection happens,
			 * silently losing the IRQ. */
		}
    }
...
--*/
{
	 ...
	 * Dieser handler ersetzt die normale Interrupbehandlung, wenn ausgef�hrt!
	 * Hierzu gibt es kein �quivalent in MONITOR, in Monitor werden daf�r IRET hooks verwendet �ber
	 * den Instruction-emulator, u.A. f�r INT, POPF, STI, ... (-> V86DispatchTable)
	 * Hooks werden generell via SetProtectedModeInterrupt BOP (Handler DpmiSetProtectedmodeInterrupt) gesetzt.
	 * Daf�r wird durch  host_iret_bop_table_addr() in ica_intack ein hook zur�ckgegeben.
	 * Diese Funktionen simulieren einen INT, statt einen echten auf der CPU auszuf�hren. Eventuell sollten wir
